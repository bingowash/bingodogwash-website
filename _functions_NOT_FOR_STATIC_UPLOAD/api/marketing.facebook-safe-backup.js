...
async function instagramPreflight(env) {
  ...
  if (!["BUSINESS", "CREATOR", "MEDIA_CREATOR"].includes(accountType)) {
    ...
    try {
    const permissions = await graphGet(INSTAGRAM_GRAPH_ORIGIN, "me/permissions", {}, tokenCheck.token);
    const permission = Array.isArray(permissions.data)
      ? permissions.data.find((item) => item?.permission === INSTAGRAM_PUBLISH_PERMISSION)
      : null;
    if (!permission) return { ok: false, authenticationOk: true, identityOk: true, error: INSTAGRAM_PERMISSION_UNCONFIRMED, api: "Instagram Login", id: String(profile.id), username: profile.username, accountType: profile.account_type, publishingPermission: "unconfirmed" };
    if (permission.status !== "granted") return { ok: false, authenticationOk: true, identityOk: true, error: META_ERROR.permission, api: "Instagram Login", id: String(profile.id), username: profile.username, accountType: profile.account_type, publishingPermission: "denied" };
    return { ok: true, authenticationOk: true, identityOk: true, api: "Instagram Login", id: String(profile.id), username: profile.username, accountType: profile.account_type, publishingPermission: "granted" };
  } catch {
    return { ok: false, authenticationOk: true, identityOk: true, error: INSTAGRAM_PERMISSION_UNCONFIRMED, api: "Instagram Login", failedCheck: "publishing-permission", id: String(profile.id), username: profile.username, accountType: profile.account_type, publishingPermission: "unconfirmed" };
  }
}
