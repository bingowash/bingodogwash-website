  ...
  if (productTarget) {
    const adminProductRows = (productList) => productList.map((product) => `
      <div class="row admin-row admin-product-row">
        ...
        <div class="admin-row-actions">
          <a class="btn btn-light" href="${adminPublicUrl(`product.html?id=${encodeURIComponent(product.id)}`)}">View public</a>
...
async function handleAdminEtsyProductAction(action) {
  ...
  if (action === "publish" && !window.confirm("Publish selected Etsy products to the public shop?")) return;
  try {
    await adminCoreJson(`${adminEtsyApiBase}/products/${encodeURIComponent(action)}`, {
      method: "POST",
...
async function adminProfessionalFetch(path, options = {}) {
  const token = (localStorage.getItem(adminProfessionalTokenKey) || "").trim();
...
async function saveAdminProfessionalToken(form) {
  ...
  localStorage.setItem(adminProfessionalTokenKey, token);
...
function initAdminProfessionals() {
  const panel = document.querySelector("[data-admin-professional-panel]");
  if (!panel) return;
  if (localStorage.getItem(adminProfessionalTokenKey)) {
    panel.hidden = false;
