...
const state = { products: [], filteredProducts: [], channels: {}, facebookPageIds: [], tiktokDirectPostEnabled: false, tiktokDirectPostReady: false, prospectingSendingEnabled: false, busy: false, currentDraftId: "" };
...
function normalizeProduct(item) { const numericPrice=Number(item.price);const price=Number.isFinite(numericPrice)&&numericPrice>0?new Intl.NumberFormat("en-GB",{style:"currency",currency:item.currency||"GBP"}).format(numericPrice):decodeEntities(item.priceLabel);return { id:String(item.id || item.externalListingId || item.external_listing_id || ""), source:String(item.source || item.supplier || "catalogue"), name:decodeEntities(item.display_title || item.title || item.name), category:decodeEntities(item.category), sku:String(item.sku || item.externalListingId || item.external_listing_id || ""), price, description:decodeEntities(item.display_description || item.description), audience:"Dog owners and dog lovers", url:safeUrl(item.publicUrl || item.public_url || item.externalUrl || item.listingUrl || item.itemUrl), image:safeUrl(item.primary_image || item.image || item.imageUrl), instagramDestinationUrl:"https://bingodogwash.com/shop" }; }
...
function prospectRow(p){const row=document.createElement("tr");const cells=[[p.business_name,p.domain],[p.business_type,p.location],[p.product_name||"No supported match",p.score?`Score ${p.score} · ${p.reason}`:""],[p.email||"No public email",p.source],[p.subscriber_type,`LIA: ${p.lia_basis_recorded?"recorded":"not recorded"}`],[p.compliance_status,""],[p.message_status||p.status,p.subject||""]];for(const [main,sub] of cells){const td=document.createElement("td");td.textContent=main||"—";if(sub){const small=document.createElement("small");small.textContent=sub;td.append(document.createElement("br"),small);}row.append(td);}const actions=document.createElement("td");actions.className="prospecting-actions";for(const [label,action,id,disabled] of [["Record LIA","record-lia",p.id,p.subscriber_type!=="corporate"||Boolean(p.lia_basis_recorded)],["Approve","approve",p.message_id,!p.message_id||p.compliance_status!=="eligible"],["Reject","reject",p.message_id,!p.message_id],["Send","send",p.message_id,!p.message_id||p.message_status!=="approved"||!state.prospectingSendingEnabled],["Block","block",p.id,false],["Suppress","suppress",p.id,false]]){const button=document.createElement("button");button.type="button";button.textContent=label;button.disabled=disabled;if(action==="send"&&!state.prospectingSendingEnabled)button.title="Brevo campaign sending is not ready or AI_EMAIL_SENDING_ENABLED is false.";button.addEventListener("click",()=>prospectingAction(action,id));actions.append(button);}row.append(actions);return row;}
async function prospectingAction(action,id){const message=qs("[data-prospecting-message]");const resource=["approve","reject","send"].includes(action)?"messages":"prospects";let body={};if(action==="record-lia"){const reference=window.prompt("Enter the reviewed LIA document/reference. This does not replace PECR consent for sole traders or unknown subscribers.");if(!reference)return;body={confirmed:true,reference};}try{const response=await fetch(`${prospectingApi}/${resource}/${encodeURIComponent(id)}/${action}`,{method:"POST",headers:authHeaders(),body:JSON.stringify(body)});const data=await response.json();if(!response.ok)throw new Error(data.error||"Action failed.");message.textContent=`Prospect action completed: ${data.status||action}.`;await loadProspecting();}catch(error){message.textContent=error.message;}}
async function runProspectingManually(){const message=qs("[data-prospecting-message]");message.textContent="Running the configured approved sources…";try{const response=await fetch(`${prospectingApi}/run`,{method:"POST",headers:authHeaders(),body:JSON.stringify({products:state.products})});const data=await response.json();if(!response.ok)throw new Error(data.error||"Prospecting run failed.");message.textContent=`Run completed: ${data.prospectsFound} found, ${data.prospectsQueued} queued, ${data.blocked} blocked.`;await loadProspecting();}catch(error){message.textContent=error.message;}}
...
qs("[data-product-select]")?.addEventListener("change",(event)=>{const product=state.products.find((item)=>item.id===event.target.value);if(product)fillProduct(product);else{const form=qs("[data-ai-draft-form]");for(const name of ["id","source","name","category","sku","price","description","audience","url","image"]){const field=form.elements.namedItem(name);if(field&&"value"in field)field.value="";}updateProductImage("");qs("[data-product-status]").textContent="Manual product selected. Enter verified product details below.";}});
...
qs("[data-prospecting-refresh]")?.addEventListener("click",loadProspecting);qs("[data-prospecting-run]")?.addEventListener("click",runProspectingManually);qs("[data-prospecting-status]")?.addEventListener("change",loadProspecting);qs("[data-prospecting-subscriber]")?.addEventListener("change",loadProspecting);

async function unlock() { if(!token())return;qs("[data-ai-workspace]").hidden=false;setMessage("Product centre unlocked for this browser session.");renderContentCards();renderChannels();renderHistories();await Promise.all([loadProducts(),loadChannels(),loadProspecting()]); }
